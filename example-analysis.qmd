---
title: "Example Analysis - Taylor vs Beyoncé"
format:
  html:
    toc: true
    toc-depth: 3
...
We're analyzing three datasets from [TidyTuesday](https://github.com/rfordatascience/tidytuesday/tree/master/data/2020/2020-09-29):
...
## 📖 Click to view detailed data dictionary
...
#| fig-cap: "Geographic distribution of album sales reveals different market strategies. Beyoncé's album sales demonstrate a more balanced global distribution, with substantial contributions from the United States, the United Kingdom, and additional international markets. In contrast, Taylor Swift's sales are predominantly concentrated in the United States, with smaller shares from the United Kingdom and other regions. These patterns indicate that Beyoncé possesses a broader international reach, whereas Taylor Swift's commercial success is more reliant on the United States market."
#| fig-height: 8
...
#| fig-cap: "Album sales trends reveal different trajectories across markets. Taylor Swift consistently achieves higher and more stable album sales than Beyoncé in the US market. In the UK market, Beyoncé initially maintained a strong presence; however, Taylor Swift's sales increased substantially over time and eventually surpassed those of Beyoncé. On a global scale, Beyoncé exhibits a more consistent appeal, whereas Taylor Swift's album sales fluctuate to a greater extent but ultimately surpass Beyoncé’s worldwide sales."
#| fig-height: 10
...
#| fig-cap: "Lyrical sentiment patterns reveal artistic differences. Beyoncé's songs demonstrate a strong concentration around neutral sentiment (score of 0) across all length categories, with relatively narrow emotional ranges. In contrast, Taylor Swift's lyrics exhibit more diverse emotional content, with notable peaks in both positive and negative territories, particularly in her medium-length songs (30+ words)."
#| fig-height: 10
...
sentiment_analysis <- bind_rows(
  ...
  mutate(
    ...
    word_count_cat = case_when(
      total_words < 30 ~ "Short Songs (Words <30)",
      total_words < 60 ~ "Medium Songs (30 < Words < 60)",
      TRUE ~ "Long Songs (Wrods > 60)"
    )
  ) %>%
...
p3 <- sentiment_analysis %>%
  ...
  labs(
    ...
    x = "Sentiment Score",
    y = "Density",
    caption = "Based on Bing sentiment lexicon | Sentiment Score = (Positive - Negative) / (Positive + Negative)"
  ) +
  # Theme
  theme_minimal(base_size = 12) +
  theme(
...
📊 Total albums: `r n_distinct(sales$title)`

📅 Date range: `r format(min(sales_clean$released, na.rm=TRUE), "%Y")` - `r format(max(sales_clean$released, na.rm=TRUE), "%Y")`

💿 Avg sales: `r round(mean(sales_clean$sales, na.rm=TRUE), 1)`M

🎵 Songs analyzed: `r n_distinct(c(b_lyrics$song_name, ts_lyrics_processed$Title))`
:::
...
1.  TidyTuesday. (2020). "Beyoncé and Taylor Swift Lyrics." [GitHub Repository](https://github.com/rfordatascience/tidytuesday/tree/master/data/2020/2020-09-29)
...
4.  IFPI. (2024). "Global Music Report 2021." International Federation of the Phonographic Industry. [Report Link](https://www.ifpi.org/our-industry/industry-data/)
